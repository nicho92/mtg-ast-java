...
public class ModalAbilityParser implements AbilityParser {
    ...
    private static final Pattern HEADER_PATTERN = Pattern.compile(
            "^Choose\\s+(one|two|one or both|up to one)\\s*[—-]?$",
    ...
    public AbilityNode parse(String text) {
        ...
        List<ModeNode> modes = lines.stream()
                .skip(1)
                .map(this::stripBullet)
                .map(modeText -> new ModeNode(
                        modeText,
                        effectSequenceParser.parse(modeText)
                ))
                .toList();
    ...
    private String stripBullet(String line) {
        ...
        return Arrays.stream(new String[] { "•", "-", "—" })
                ...
                .findFirst()
                .map(marker -> line.substring(marker.length()).trim())
                .orElse(line);
    }
}
